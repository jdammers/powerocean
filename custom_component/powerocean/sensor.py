...
from .ecoflow import EcoflowApi, AuthenticationFailed
...
async def async_setup_entry(hass, config_entry, async_add_entities):
    ...
    try:
        ...
        if not auth_check:
            ...
            _LOGGER.warning(
                f"{serial}: It appears the PowerOcean device is offline or has changed host."
                + ISSUE_URL_ERROR_MESSAGE
            )

    except AuthenticationFailed as error:
        _LOGGER.warning(f"{serial}: Authentication failed: {error}")
        return
    ...
    try:
        ...
        if not data:
            ...
            _LOGGER.warning(
                f"{serial}: Failed to fetch sensor data - authentication failed or no data. {data}"
                + ISSUE_URL_ERROR_MESSAGE
            )
            return
    ...
    # Exception if data cannot be fetched
        ...
        _LOGGER.warning(
            f"{serial}: Failed to fetch sensor data: {error}"
            + ISSUE_URL_ERROR_MESSAGE
        )
        return
    ...
    _LOGGER.debug(f"{serial}: Device ID: {device_id}")
    ...
    _LOGGER.debug(f"{serial}: List of device_specific_sensors[device_id]: "
                  f"{device_specific_sensors[device_id]}")

    # Log the number of sensors registered (and added to the update list)
    _LOGGER.debug(f"{serial}: All '{len(device_specific_sensors[device_id])}' sensors have registered.")
    ...
    async def async_update_data(now):
        ...
        _LOGGER.debug(f"{serial}: Preparing to update sensors at {now}")
        ...
        try:
            ...
            _LOGGER.error(f"{serial}: Error fetching data from the device: {e}" + ISSUE_URL_ERROR_MESSAGE )
            return
        ...
        if device_id in hass.data.get(DOMAIN, {}).get("device_specific_sensors", {}):
            ...
            for sensor in device_specific_sensors[device_id]:
                ...
                if entity_id:
                    ...
                    if entity and not entity.disabled_by:
                        ...
                        _LOGGER.debug(f"{serial}: Sensor {sensor.name} enabled.")
                        if sensor_data:
                            _LOGGER.debug(f"{serial}: Sensor {sensor.name} has API data to update {sensor_data}")
                            ...
                            if (str(sensor._state).strip() != str(sensor_data.value).strip()):
                                _LOGGER.debug(
                                    f"{serial}: Sensor {sensor.name} marked for update: current state = "
                                ...
                                _LOGGER.debug(f"{serial}: Sensor {sensor.name} skipped update! Current value = "
                                              f"{sensor._state}, new value = {sensor_data.value}"
                                )
                                counter_unchanged = counter_unchanged + 1
                        else:
                            _LOGGER.warning(
                                f"{serial}: Sensor {sensor.name}: found no data for update!" + ISSUE_URL_ERROR_MESSAGE
                            )
                            counter_error = counter_error + 1
                    else:
                        _LOGGER.debug(f"{serial}: Sensor {sensor.name} is disabled, skipping update")
                        counter_disabled = counter_disabled + 1
                else:
                    _LOGGER.warning(
                        f"{serial}: Sensor {sensor.name} not found in the registry, skipping update"
                        + ISSUE_URL_ERROR_MESSAGE
                    )
                    counter_error = counter_error + 1
            ...
            _LOGGER.debug(
                f"{serial}: A total of {counter_updated} sensors have been updated. "
        ...
        # Device not in list: must have been deleted, will resolve post re-start
            ...
            _LOGGER.warning(f"{serial}: Sensor must have been deleted, re-start of HA recommended.")
...
class PowerOceanSensor(SensorEntity):
    ...
    def __init__(self, ecoflow: EcoflowApi, endpoint):
